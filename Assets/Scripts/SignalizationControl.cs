...
public class SignalizationControl : MonoBehaviour
{
    ...
    public void UseSignalization()
    {
        ...
        {
            _signalizationCoroutine = StartCoroutine(StartSignal(_minVolume, _maxVolume, _signalizationClip.length, true));
        }
        else
        {
            _signalizationCoroutine = StartCoroutine(StartSignal(_audioSource.volume, _minVolume, _signalizationClip.length, false));
        }
    }
